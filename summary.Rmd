...
# NOTE: The youngest `account_age` is 7 days, and the oldest account is 14 years (5259 days). 

```

```{r eval=FALSE, include=FALSE}
# **How many accounts are younger than 2 weeks?**
# NOTE: 4% (19) of the accounts are younger than two weeks old, and all of them address @foxnews. 
...
#NOTE: 10 have neutral support, 5 supports and 4 are against. We will look into the tweets to verify further. 

tweets %>% filter(account_age < 14) %>% group_by(experiment_group, opinion_label) %>% summarize(younger_than_two_weeks = n())
```

```{r eval=FALSE, include=FALSE}
# NOTES: Given the low agreement score from the annotation, it's expected that the tweets aren't clear cut for/against/neutral on student loan. 
...
## **A) Are the opinions more polarized when they are engaging/associating with conservative sources (Fox) or liberal sources (CNN, MSNBC, NPR, NYTimes)?** 
...
library(reshape2)
c <- tweets %>% 
  group_by(opinion_label, ego_involvement_label) %>% 
  summarize(n = n())
